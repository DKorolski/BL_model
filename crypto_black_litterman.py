...
mcap=pd.read_csv('mcap_crypto.csv',  index_col=[0])
tickers=['BTC-USD',
'ETH-USD',
'ADA-USD',
'BNB-USD',
...
ticker = 'IMOEX.ME'
btc_prices = yf.download(ticker, start=start_date, end=end_date)["Adj Close"]
print(btc_prices)

prices=pd.DataFrame(
    index=pd.date_range(start_date,end_date1,freq='D')
)
prices=prices[prices.index.isin(btc_prices.index)]
result=pd.DataFrame(
...
tickers=mcap['ticker']
print(tickers)
#tickers=['BTC-USD',  'LTC-USD',   'XMR-USD']
for i in tickers:
    if z<=30:
        ticker=tickers.values[z]
       # ticker=tickers[z]
        try:
            ...
            market_prices1=market_prices1[market_prices1.index.isin(btc_prices.index)]
            market_prices1=market_prices1.dropna()
            asset_df= market_prices1.loc[:,ticker].sort_index()
            if len(asset_df)>=0.8*len(btc_prices) and asset_df.values[0] >0.2 and asset_df.values[-1] >0.2 and (asset_df.values[-1]/asset_df.values[0]) > 1:
...
print(prices.head(2))
print(result.head(2))
btc_prices=btc_prices[btc_prices.index.isin(prices.index)]
prices=prices[prices.index.isin(btc_prices.index)]
print(prices)
prices.to_csv('prices111.csv')
result.to_csv('result111.csv')
selected_tickers=prices.columns.tolist()
print(selected_tickers)
prices=pd.read_csv('prices11.csv', index_col=[0], parse_dates=True)
result=pd.read_csv('result11.csv', index_col=[0,1], parse_dates=True)
...
delta = pyp.black_litterman.market_implied_risk_aversion(btc_prices, risk_free_rate=0.05796)
print(delta)
...
#get views from file
view_confidence = pd.read_csv('view_crypto.csv', delimiter=';', index_col=[0])
...
benchmark = '^CMC200' # S&P BSE Sensex
